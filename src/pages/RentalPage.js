...
const RentalPage = () => {
  ...
  return (
    <section className="w-full container px-4 py-8 flex justify-center">
      {data ?
        <Box sx={{ width: '100%', maxWidth: "900px" }}>
          ...
          <form onSubmit={formik.handleSubmit} className="w-full pt-8">
            {/* step 1 */}
            {activeStep === 0 && <BoxDetailRental data={data} lengthSteps={steps.length} />}
            {/* step 2 */}
            <div className={`${activeStep === 1 ? "flex" : "hidden"} w-full max-w-[900px] mx-auto select-none bg-white dark:bg-zinc-800 rounded-[10px] shadow p-4 gap-1 flex-wrap`}>
            ...
            {/* step 3 */}
            <div className={` ${activeStep === 2 ? "flex" : "hidden"} w-full max-w-[900px] mx-auto flex gap-4 flex-wrap select-none bg-white dark:bg-zinc-800 rounded-[10px] shadow p-4`}>
              <div className="flex w-full items-center justify-between flex-wrap dark:text-white">
